...
CFLAGS = -fPIC -I$(ERL_INCLUDE_PATH) -O3 -std=c++14 -Wall -Wextra
LDFLAGS =
...
ifeq ($(findstring linux,$(CXX)),linux)
	COMPILE_ARCH=linux
endif
...
SOURCES := $(shell find $(SRC_DIRECTORY) -name *.cpp)

# Lists of all the C header files.
HEADERS := $(shell find $(INCLUDE_DIRECTORY) -name *.hpp)
...
NIFS_SOURCES := $(wildcard $(NIFS_DIRECTORY)/*.cpp)

# A list of all the helpers used by the NIF code files.
NIFS_HELPERS := $(shell find $(NIFS_DIRECTORY) -name *_helper.cpp)

# A list of all the shared objects created by compiling the NIF code files.
NIFS_OBJECTS := $(NIFS_SOURCES:$(NIFS_DIRECTORY)/%.cpp=$(PRIV_DIRECTORY)/%.so)

# A list of all the shared objects created from the C code files with the test
# specific flags.
TEST_OBJECTS := $(SOURCES:$(SRC_DIRECTORY)/%.cpp=$(TEST_OBJ_DIRECTORY)/%.o)
...
#   ```
#
$(OBJECTS): $(OBJ_DIRECTORY)/%.o : $(SRC_DIRECTORY)/%.cpp $(INCLUDE_DIRECTORY)/%.hpp
	@echo 'Compiling: '$<
	@$(CXX) $(CFLAGS) -c $< -o $@
...
#   ```
#
$(NIFS_OBJECTS): $(PRIV_DIRECTORY)/%.so : $(NIFS_DIRECTORY)/%.cpp $(OBJECTS) $(NIFS_HELPERS)
	@echo 'Creating NIF: '$@
...
# 	$(CXX) $(TEST_CFLAGS) $(TEST_OBJECTS) -o test/c/temp/test test/test_helper.c $(LDFLAGS) $(TEST_LDFLAGS)
# 	./test/c/temp/test
...
#
clean:
	@$(RM) -rf $(OBJ_DIRECTORY) $(TEST_OBJ_DIRECTORY)
	@$(RM) -rf $(PRIV_DIRECTORY)/*.o $(PRIV_DIRECTORY)/*.so
